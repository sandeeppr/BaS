	...
	$(document)
			.ready(
					function() {
						...
						function proceed_toStep2(response) {
							...
							//alert(finalData);
							$('#search_results')
									.html("<form id='searchform' class='form-horizontal' role='form'>");					
							$('#search_results')
									.append("<ul id='search_results' class='thumbnails' style='list-style-type: none;'>");
							$.each(finalData,
									function(index, value) {	
											//alert(value);								
											console.log(value);
											$("#search_results").append("<li class='span6' style='list-style-type: none;'>"
											+"<div class='thumbnail right-caption span6'>"
												...
												+"<div class='caption'>"
													...
													+"<p><span class='label label-warning'><small>Our Rating : "+value.rating+" &nbsp;&nbsp;&nbsp;<span class='glyphicon glyphicon-star'></span> </small> </span></p>"
													+"<hr>"
													+"<p class='span4'>Some text to put it in his own words to explain his services. Or we can put it up.</p>"
													+"<p>"
														+"<button id='btn-selectme' type='button' value='"+value.provider+"_"+value.tariff+"' class='btn btn-info pull-right'><i class='icon-hand-right'></i>Select & Proceed </button>"
													+"</p></div></div></li>"
															);
																	//"<li class='span4' style='list-style-type: none;'><div class='thumbnail right-caption span4'><img class='span2' src='http://placehold.it/120x160' alt=''> <div class='caption'><h5>"+value.name+"</h5><p>"+value.locality+"</p><p>"+value.badges.LANG+"</p><p>"+value.badges.EXPR+"</p> <p><a href='#' class='btn btn-primary'>Action</a> </p> </div> </div> </li>");
			...
			return json;
					...
					$('#contact_no').blur(
						...
						if(name != ""  && name.length > 9 && name.length < 12 && $.isNumeric(name)){
							$('#contact_no_error').html("");
							$('#contact_no_error').css('display', 'none');
							
							$('#pickup').focus();
						}else{
							$('#contact_no_error').html("Contact number must be numeric and not more than 11 digits.");
					...
					$('#btn-sbscibe').click(function(e){
			    		...
			    		$.ajax({
							type : "GET",
							url : "/BaS/sahaay/auth/"+sub_id+"/subscribe",
							cache : false,
							dataType : "text",
								...
								<form id="form_search" class="form-horizontal" action="/sahaay/metadata/search">
									...
									<div class="form-group">
										...
										<div class="col-md-offset-2 col-md-8">
											<button id="btn-search" type="button" class="btn btn-info" disabled='disabled'>
