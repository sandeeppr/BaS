...
$(document).ready(function() {
    ...
    $('ul.setup-panel li.active a').trigger('click');    
     ...
     $('#btn-complete_profile').on('click', function(e) {
     	...
     	var hasError = false;
		//var comma = ", ";
		var errorMsg = "Please provide valid values for ";
		//$('#detailsalert').html("Please provide valid values for ");
		
		var pic = $('#pic_file').val();
		if(pic == "" || pic.length < 1){			
			hasError = true;
			errorMsg = errorMsg+"Profile Pic, ";	
			//alert(errorMsg);
		}
		
		var dl_file = $('#dl_file').val();
		if(dl_file == "" || dl_file.length < 1){			
			//if(hasError == false) comma = "";
			hasError = true;
			errorMsg = errorMsg+"Driving License, ";			
		}
		
		var pan_file = $('#pan_file').val();
		if(pan_file == "" || pan_file.length < 1){			
			//if(hasError == false) comma = "";
			hasError = true;
		...
		if(langs == "" || langs.length < 1){
			//if(hasError == false) comma = "";
			hasError = true;
			errorMsg = errorMsg+"Languages, ";			
		}
		
		var expVal1 = $('#exprv1').val();
		var expVal2 = $('#value2').val();
		var expVal3 = $('#value3').val();

		if(expVal1 == "" && expVal2 == "" && expVal3 == ""){
			//if(hasError == false) comma = "";
			hasError = true;
			errorMsg = errorMsg+"Experience Details, ";	
		}
		...
		if(line_1 == "" || line_1.length < 4){
			//if(hasError == false) comma = "";
			hasError = true;
			errorMsg = errorMsg+"Address Line1, ";		
		}
		...
		if(line_2 == "" || line_2.length < 4){
			...
			//if(hasError == false) comma = "";
			hasError = true;
			errorMsg = errorMsg+"Address Line2, ";
		}
		
		if($('#addrcity').val() == "xxxx"){
			//if(hasError == false) comma = "";
			hasError = true;
		...
		if(hasError == true){
			...
			var finalErrMsg = errorMsg.substring(0, errorMsg.length-2);
			
			finalErrMsg = finalErrMsg+" to proceed !!";
			$('#detailsalert').html(finalErrMsg);
			$('#detailsalert').css('display', 'block');
		}else{
			
			$('#detailsalert').html("");
			$('#detailsalert').css('display', 'none');
     	...
     	console.log(JSON.stringify($('#upload_form').serializeObject()));
   		alert(JSON.stringify($('#upload_form').serializeObject())); 
     	$.ajax({
					type : "POST",
					url : "/sahaay/profileMgmt/uploadProfile",
					data : JSON.stringify($('#upload_form').serializeObject()),
    ...
    $('#btn-register').on('click', function(e) {
    	...
    	var cityId = $("#city option:selected").val();
   		...
   		var customer_pwd = $("#password").val();
   		var customer_mobile = $("#mobile").val();
   		var customer_type = $("#type").val();
   		var customer_status = $("#status").val();
   		...
   		$("#user_type").val(customer_type);
   		$('#user_status').val(customer_status);
   		$("#user_password").val(customer_pwd);
   		$("#user_service_id").val(svrId);
   ...
   var badgesCount = 0;
					...
					$("#service_id").change(
										function(event) {
											...
											if (svrId == 'Driver') {
												...
												$('#pref1')
														.html(
																"<div class='form-group'><label for='param4' class='col-md-4 control-label'>Experience details&nbsp;&nbsp;&nbsp;<span style='color: red;'>*</span></label><div class='col-md-2'><input type='text' class='form-control' id='expr1' name='badgesOwned.param1' disabled='disabled' value='EXPR' />&nbsp;</div><div class='col-md-1'><input class='form-control' type='text' id='exprv1' name='badgesOwned.value1' placeholder='years'/></div></div> ");
												$('#pref1')
														.append(
																"<div class='form-group'><label for='param4' class='col-md-4 control-label'></label><div class='col-md-2'><input type='text' class='form-control' id='expr1' name='badgesOwned.param1' disabled='disabled' value='EXP_PREMIUM' />&nbsp;</div><div class='col-md-1'><input class='form-control' type='text' placeholder='years' id='value2' name='badgesOwned.value2' /></div></div> ");
												$('#pref1')
														.append(
																"<div class='form-group'><label for='param4' class='col-md-4 control-label'></label><div class='col-md-2'><input type='text' class='form-control' id='expr1' name='badgesOwned.param1' disabled='disabled' value='EXP_SUPER_PREMIUM' />&nbsp;</div><div class='col-md-1'><input class='form-control' type='text' placeholder='years' id='value3' name='badgesOwned.value3' /></div></div> ");
				 ...
				 /* $('#uploadBtn').on('click',function(event){						
					...
					$('#pic_file').change(function(event){
						//alert($('#pic_file').val());
						var hasFile = $('#pic_file').val();
						if(hasFile != ""){
							var ufile = document.upload_form.pic_file;
							if(ufile.type == "file") {
								var actualFile = ufile.files[0];
								//alert('size '+actualFile.size);
								console.log(ufile);
								var blnValid = false;
					...
					$('#mobile').blur(function(){
						...
						//alert('starts with 0'+mobileReg.test(mobile));
						...
						if(mobileReg.test(mobile)){ 
							...
							$('#mobile_error').html("");
							$('#mobile_error').html("Please provide a valid mobile number. It must not start with a zero.");
							$('#mobile_error').css('display', 'block');	
						}else {
							$('#mobile_error').html("");
							$('#mobile_error').css('display', 'none');
						
							if(mobile != "" && $.isNumeric(mobile) && mobile.length == 10 && !mobileReg.test(mobile)){
								$('#shadow').addClass('blocker');  
								$.ajax({
									type : "GET",
									...
									error : function(xhr) {
										...
										$('#mobile_error').html("Mobile number must be numeric and not more than 10 digits.");
										$('#mobile_error').css('display', 'block');
							}
						}
    ...
    <table align="center" border="0" cellpadding="0" cellspacing="0" width="1003">
      <tbody><tr>
        <td style="width: 646px;" class="menu_nav" height="40">
							...
							<td style="width: 300px;" height="40" ></td>
							...
							<div class="span6">
								...
								<f:form id="upload_form" name="upload_form" class="form-horizontal" action="/sahaay/profileMgmt/uploadFile" method="post" modelAttribute="prof" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="form-group">	
											<input name="user.id" id="user_id" value="" type="hidden" />
											<input name="user.password" id="user_password" value="" type="hidden" />
											<input name='user.name' id="user_name" value="" type="hidden" />		
											<input name="user.mobile" id="user_mobile" value="" type="hidden" />
											<input name="user.type" id="user_type" value="" type="hidden" />
											<input name='user.status' id="user_status" value="" type="hidden" />	
											<input name='user.service_id' id="user_service_id" value="" type="hidden" />													
										...
										<div class="form-group">
											...
											<div class="col-md-4">
													...
													name="address.line_1"   placeHolder="address line1"/>
										...
										<div class="form-group">
											...
											<div class="col-md-4">
													...
													name="address.line_2"  placeHolder="address line2"/>
										...
										<div class="form-group">
											<label for="pin" class="col-md-4 control-label"><span style="color: red;">*</span></label>
												<div class="col-md-2">
													...
													<select class="form-control" id='addrcity' name="address.city"  
													placeholder="City">
													<option value="xxxx">Select a city</option>
													<option value="Hyderabad">Hyderabad</option>
													<option value="Bengaluru">Bangalore</option>
											...
											<div class="col-md-2">
													...
													name="address.zipcode"  placeHolder="zipcode"/>
										...
										<div class="form-group">
											...
											<div class="col-md-2">													
													<select class="form-control" id='addrstate' name="address.state"  
													placeholder="State">
											...
											<div class="col-md-2">		
													<input type='text' class="form-control" id="addrcountry"
													name="address.country" value="India"  placeHolder="zipcode" disabled="disabled"/>											
										</div>
										</div>
										...
										<div class="form-group">
											<!-- Button -->
											<div class="col-md-offset-4 col-md-4">
												<button id="btn-complete_profile" type="button" class="btn btn-primary">
